...
<%!String maxUserName = "", customerName = ""; double max; LinkedList<String> maxSold; int size; PreparedStatement p; ResultSet rs; Connection con;%>
		...
		<% try {
			...
			<%
					...
					p = con.prepareStatement("select ticketnumber maxTickets from temp order by numSold DESC;");
					rs = p.executeQuery();
				...
				while (rCust.next()) { %>
					<tr>
						<form action=EditAccounts.jsp method=post>
								...
								<td><input type=text value=<% out.println(rCust.getString("CustomerName")); %> name=NewCustomerName>
								<td><input type=text value=<% out.println(rCust.getString("Password")); %> name=NewCustomerPassword>
				...
				while (rrep.next()) { %>
					<tr>
						<form action=EditAccounts.jsp method=post>
								...
								<td><input type=text value=<% out.println(rrep.getString("Password")); %> name=NewRepPassword>
								<input type=submit value="Edit password">
						</td>
						</form>
						<form action=EditAccounts.jsp method=post>
						<td><input value=<% out.println(rrep.getString("Username")); %> name=DeleteRepUsername readonly>
			...
			<form action=EditAccounts.jsp method=post>
			<td><input name=AddRepUsername >
			<td><input name=AddRepPassword >
			<input type=submit value="Add">
			</form>
			</table>
